...
import { deleteSavingApi } from '@/composables/api/savings/delete.api';
...
const onDelete = async (reason: string) => {
  ...
  try {
    await deleteSavingApi(_id.value as string, reason);
    toast(`Delete Saving "${label.value}" success`, { color: 'success' });
    emit('deleted');
  } catch (error) {
