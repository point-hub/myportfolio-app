...
import { deleteDividendStockApi } from '@/composables/api/dividend-stocks/delete.api';
...
const onDelete = async (reason: string) => {
  ...
  try {
    await deleteDividendStockApi(_id.value as string, reason);
    toast(`Delete Dividend Stock "${label.value}" success`, { color: 'success' });
    emit('deleted');
  } catch (error) {
