...
import { restorePaymentStockApi } from '@/composables/api/payment-stocks/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restorePaymentStockApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Payment Stock "${label.value}" success`, { color: 'success' });
    emit('restored');
