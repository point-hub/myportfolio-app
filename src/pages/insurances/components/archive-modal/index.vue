...
import { archiveInsuranceApi } from '@/composables/api/insurances/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveInsuranceApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Insurance "${label.value}" success`, { color: 'success' });
    emit('archived');
