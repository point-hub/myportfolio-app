...
import { restoreBankApi } from '@/composables/api/master/banks/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreBankApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Bank "${label.value}" success`, { color: 'success' });
    emit('restored');
  } catch (error) {
