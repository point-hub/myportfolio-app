...
import { getSavingsApi, type ISavingData } from '@/composables/api/savings/get.api';
...
const {
  ...
  columns: {
    ...
    'placement.bank.name': { label: 'Bank Placement - Name', isVisible: true, isSelectable: true },
    'source.bank.name': { label: 'Bank Source - Name', isVisible: true, isSelectable: true },
    'source.bank.account.account_number': { label: 'Bank Source - Account Number', isVisible: true, isSelectable: true },
    'source.bank.account.account_name': { label: 'Bank Source - Account Name', isVisible: true, isSelectable: true },
...
const {
  ...
  initialFilter: {
    ...
    'placement.bank.name': '',
    'source.bank.name': '',
    'source.bank.account.account_name': '',
    'source.bank.account.account_number': '',
    'interest.bank_name': '',
    'interest.bank_account_name': '',
  ...
  initialSortKeys: {
    ...
    'placement.amount': 0,
    'placement.bank.name': 0,
    'source.bank.name': 0,
    'source.bank.account.account_name': 0,
    'source.bank.account.account_number': 0,
 ...
 * @param saving - The data row to delete
 * @param index - Index of the row for UI references
 */
const onDeleteModal = (saving: ISavingData, index: number) => {
  rowMenuRef.value[index].toggle(false);
  deleteModalRef.value.toggleModal({
    _id: saving._id,
    label: `${saving.form_number}`,
  });
};
...
const getWithdrawalAmount = (saving: ISavingData) => {
      ...
      <base-table>
        <thead>
          ...
          <tr class="bg-slate-100 dark:bg-slate-700">
            ...
            <th v-if="columns['group.name']?.isVisible">
              <base-input v-model="filter['group.name']" placeholder="Search..." :readonly="isLoading" border="none" paddingless />
            </th>
            ...
            <th v-if="columns['placement.bank.name']?.isVisible">
              <base-input v-model="filter['placement.bank.name']" placeholder="Search..." :readonly="isLoading" border="none" paddingless />
            </th>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && savings && savings.length > 0">
            <tr v-for="(saving, index) in savings" :key="index" :class="{'bg-red-50 dark:bg-red-800': saving.status === 'draft'}">
              <td>
                <!-- Row action menu -->
                  ...
                  <template #content>
                    <base-card class="p-0! gap-0! -mt-2" shadow>
                      <div class="flex flex-col">
                        <router-link :to="`/savings/${saving._id}`">
                        ...
                        <router-link v-if="authStore.hasPermission('savings:update')" :to="`/savings/${saving._id}/edit`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
                            <base-icon icon="i-fa7-light-file-pen" />
                            <p class="flex-1">Edit</p>
                          </base-button>
                        </router-link>
                        <router-link v-if="authStore.hasPermission('savings:renew') && saving.status === 'active'" :to="`/savings/${saving._id}/extend`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
                            <base-icon icon="i-fa7-light-file-pen" />
                        ...
                        <base-button v-if="authStore.hasPermission('savings:delete')" @click="onDeleteModal(saving, index)" variant="text" color="danger" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
              ...
              <td v-if="columns['withdrawal.status']?.isVisible">
                ...
                <base-badge v-else-if="saving.withdrawal?.received_date && (saving.withdrawal?.remaining_amount ?? 0) <= 0" variant="filled" color="success" class="font-bold w-32 uppercase">
              ...
              <td v-if="columns['group.name']?.isVisible">
                <router-link :to="`/master/owners/${saving.group?._id}`" class="text-blue">{{ saving.group?.name }}</router-link>
              </td>
              ...
              <td v-if="columns['withdrawal.received_amount']?.isVisible">{{ formatNumber(saving.withdrawal?.received_amount, 4) }}</td>
              <td v-if="columns['withdrawal.remaining_amount']?.isVisible">{{ formatNumber(saving.withdrawal?.remaining_amount, 4) }}</td>
