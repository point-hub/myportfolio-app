...
import { archiveStockApi } from '@/composables/api/stocks/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveStockApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Stock "${label.value}" success`, { color: 'success' });
    emit('archived');
