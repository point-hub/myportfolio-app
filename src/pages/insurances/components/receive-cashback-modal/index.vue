...
import { deleteCashbackInsuranceApi } from '@/composables/api/insurances/delete-cashback.api';
import { receiveCashbackInsuranceApi } from '@/composables/api/insurances/receive-cashback.api';
...
const onReceive = async () => {
  ...
  try {
    await receiveCashbackInsuranceApi(data.value._id as string, {
...
const onDelete = async () => {
  ...
  try {
    await deleteCashbackInsuranceApi(data.value._id as string, {
      uuid: data.value?.uuid,
    });
    toast('Delete cashback success', { color: 'success' });
    emit('received');
    reset();
    confirmActionModalRef.value.toggleModal(false);
  } catch (error) {
...
<template>
  ...
  >
    <div class="flex flex-col gap-4">
      ...
      <base-input-number layout="v" label="Payment Amount" align="left" v-model="data.amount" disabled decimal-length="4" />
      ...
      <base-input-number layout="v" label="Received Amount" required align="left" v-model="data.received_amount" :errors="errors.received_amount" :disabled="isReceiving || readonly" decimal-length="4" />
      <hr class="border-slate-300 dark:border-slate-600"  />
      <base-select
        layout="v"
      ...
      <base-input-number v-if="isAddAdditionalPayment" layout="v" label="Received Additional Payment Amount" align="left" v-model="data.received_additional_payment_amount" :errors="errors.additional_received_amount" :disabled="isReceiving || readonly" decimal-length="4" />
