...
import { archiveBrokerApi } from '@/composables/api/master/brokers/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveBrokerApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Broker "${label.value}" success`, { color: 'success' });
    emit('archived');
  } catch (error) {
