...
import { createSavingApi } from '@/composables/api/savings/create.api';
import { draftSavingApi } from '@/composables/api/savings/draft.api';
...
const onSaveAsDraft = async () => {
  try {
    isSaving.value = true;
    const response = await draftSavingApi(form.data);
...
const onSave = async () => {
  try {
    isSaving.value = true;
    const response = await createSavingApi(form.data);
    if (response?.inserted_id) {
      toast('Create success', { color: 'success' });
      await router.push(`/savings/${response.inserted_id}`);
    }
