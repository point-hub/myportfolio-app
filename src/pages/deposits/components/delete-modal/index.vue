...
import { deleteDepositApi } from '@/composables/api/deposits/delete.api';
...
const onDelete = async (reason: string) => {
  ...
  try {
    await deleteDepositApi(_id.value as string, reason);
    toast(`Delete Deposit "${label.value}" success`, { color: 'success' });
    emit('deleted');
  } catch (error) {
