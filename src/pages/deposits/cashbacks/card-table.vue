          ...
          <template v-if="!isLoading && deposits && deposits.length > 0">
            <template v-for="(deposit, index) in deposits" :key="index">
              <tr :class="{'bg-red-50 dark:bg-red-800': deposit.status === 'draft'}">
                ...
                <td class="text-right" v-if="columns['cashback_schedule.amount']?.isVisible">{{ formatNumber(deposit.cashback_schedule?.amount, 3) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.received_amount']?.isVisible">{{ formatNumber(deposit.cashback_schedule?.received_amount, 3) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.received_additional_payment_amount']?.isVisible">{{ formatNumber(deposit.cashback_schedule?.received_additional_payment_amount, 3) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.remaining_amount']?.isVisible">{{ formatNumber(roundNumber((deposit.cashback_schedule?.amount ?? 0) - (deposit.cashback_schedule?.received_amount ?? 0) - (deposit.cashback_schedule?.received_additional_payment_amount ?? 0), 3), 3) }}</td>
