...
const owner = defineModel<IForm>('data');

/**
 * Opens the archive confirmation modal for a specific owner.
 */
const onArchiveModal = () => {
  archiveModalRef.value.toggleModal({
    _id: owner.value?._id,
    label: `[${owner.value?.code}] ${owner.value?.name}`,
  });
};
...
/**
 * Opens the restore confirmation modal for a specific owner.
 */
...
const onDeleted = async () => {
  await router.push('/master/owners');
};
</script>
...
<template>
  ...
  <base-card class="py-3! gap-0!">
    <div class="flex gap-2 overflow-auto scrollbar-hidden">
      ...
      <router-link v-if="authStore.hasPermission('owners:update') && owner?._id" :to="`/master/owners/${route.params.id}/edit`">
        <base-button variant="filled" color="primary" size="sm" class="font-bold">
          <base-icon icon="i-fa7-solid:file-pen" /> EDIT
        </base-button>
      </router-link>
      <router-link v-if="authStore.hasPermission('owners:read')" :to="`/master/owners/${route.params.id}/histories`">
        <base-button variant="filled" color="primary" size="sm" class="font-bold">
          <base-icon icon="i-fa7-solid:rectangle-history-circle-user" /> HISTORIES
        </base-button>
      </router-link>
      <router-link v-if="authStore.hasPermission('audit-logs:read')" :to="`/master/owners/${route.params.id}/audits`">
        <base-button variant="filled" color="primary" size="sm" class="font-bold">
          <base-icon icon="i-fa7-solid:file-magnifying-glass" /> AUDITS
        </base-button>
      </router-link>
      <base-button v-if="authStore.hasPermission('owners:delete') && owner?._id" @click="onDeleteModal" variant="filled" color="primary" size="sm" class="font-bold">
        <base-icon icon="i-fa7-solid:trash-xmark" /> DELETE
      </base-button>
      <base-button v-if="authStore.hasPermission('owners:update') && owner?._id && !owner.is_archived" @click="onArchiveModal" variant="filled" color="primary" size="sm" class="font-bold">
        <base-icon icon="i-fa7-solid:box-archive" /> ARCHIVE
      </base-button>
      <base-button v-if="authStore.hasPermission('owners:update') && owner?._id && owner.is_archived" @click="onRestoreModal" variant="filled" color="primary" size="sm" class="font-bold">
