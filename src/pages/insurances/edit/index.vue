...
import { updateInsuranceApi } from '@/composables/api/insurances/update.api';
import { updateDraftInsuranceApi } from '@/composables/api/insurances/update-draft.api';
...
const onUpdate = async () => {
  try {
    isSaving.value = true;
    const response = await updateInsuranceApi(route.params.id as string, form.data);
    if (response?.matched_count) {
      toast('Update success', { color: 'success' });
      await router.push(`/insurances/${route.params.id}`);
    }
  } catch (error) {
    const errorResponse = handleError(error);
...
const onUpdateDraft = async () => {
  try {
    isSaving.value = true;
    const response = await updateDraftInsuranceApi(route.params.id as string, form.data);
    if (response?.matched_count) {
      toast('Update success', { color: 'success' });
      await router.push(`/insurances/${route.params.id}`);
    }
