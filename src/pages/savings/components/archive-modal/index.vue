...
import { archiveSavingApi } from '@/composables/api/savings/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveSavingApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Saving "${label.value}" success`, { color: 'success' });
    emit('archived');
