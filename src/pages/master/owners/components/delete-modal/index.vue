...
import { deleteOwnerApi } from '@/composables/api/master/owners/delete.api';
...
const onDelete = async (reason: string) => {
  ...
  try {
    await deleteOwnerApi(_id.value as string, reason);
    toast(`Delete Owner "${label.value}" success`, { color: 'success' });
    emit('deleted');
  } catch (error) {
