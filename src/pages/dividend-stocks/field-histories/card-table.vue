      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && auditLogs && auditLogs.length > 0">
            <tr v-for="(dividendStock, index) in auditLogs" :key="index">
              <!-- Fields rendered conditionally based on column visibility -->
              <td v-if="columns['created_at']?.isVisible" class="w-40">{{ formatDate(dividendStock.created_at) }}</td>
              <td v-if="columns['operation_id']?.isVisible">
                <router-link :to="`/administrator/audit-logs/${dividendStock.operation_id}`" class="text-blue">{{ dividendStock.operation_id?.substring(0, 13) }}...</router-link>
              </td>
              <td v-if="columns['actor']?.isVisible">
                <router-link :to="`/master/users/${dividendStock.actor_id}`" class="text-blue">{{ dividendStock.actor_name }}</router-link>
              </td>
              <td v-if="columns['before']?.isVisible">{{ dividendStock.before }}</td>
              <td v-if="columns['after']?.isVisible">{{ dividendStock.after }}</td>
            </tr>
          </template>
        </tbody>
      </base-table>
