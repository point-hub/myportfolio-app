...
import { createInsuranceApi } from '@/composables/api/insurances/create.api';
import { draftInsuranceApi } from '@/composables/api/insurances/draft.api';
...
const onSaveAsDraft = async () => {
  try {
    isSaving.value = true;
    const response = await draftInsuranceApi(form.data);
...
const onSave = async () => {
  try {
    isSaving.value = true;
    const response = await createInsuranceApi(form.data);
    if (response?.inserted_id) {
      toast('Create success', { color: 'success' });
      await router.push(`/insurances/${response.inserted_id}`);
    }
