...
import { restoreDividendStockApi } from '@/composables/api/dividend-stocks/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreDividendStockApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Dividend Stock "${label.value}" success`, { color: 'success' });
    emit('restored');
