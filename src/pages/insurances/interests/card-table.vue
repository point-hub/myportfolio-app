...
import { getInsuranceInterestsApi, type IInsuranceData } from '@/composables/api/insurances/get-interests.api';
...
const {
  ...
  columns: {
    ...
    'placement.policy_number': { label: 'Policy Number', isVisible: true, isSelectable: true },
 ...
 * - insurances data retrieved from API
 * - loading state
 * - control flags to prevent unnecessary watcher triggers
 */
const insurances = ref<IInsuranceData[]>();
...
const onPageUpdate = async () => {
  if (!isInitialSetup.value) {
    await getInsurances(pagination.page);
 ...
 */
const getInsurances = async (page = 1) => {
  try {
    isLoading.value = true;
    const response = await getInsuranceInterestsApi({
      search: filter,
      ...
      <base-table>
        <thead>
          ...
          <tr class="bg-slate-100 dark:bg-slate-700">
            ...
            <th v-if="columns['placement.policy_number']?.isVisible">
              <base-input v-model="filter['placement.policy_number']" placeholder="Search..." :readonly="isLoading" border="none" paddingless />
            </th>
        ...
        <tbody>
          ...
          <!-- Show no data found message if no insurances and query params exist -->
          <tr v-if="!isLoading && insurances?.length === 0 && route.query">
            <td :colspan="countVisibleColumns + 1">
          ...
          <!-- Render rows of insurance data when available -->
          <template v-if="!isLoading && insurances && insurances.length > 0">
            <template v-for="(insurance, index) in insurances" :key="index">
              <tr :class="{'bg-red-50 dark:bg-red-800': insurance.status === 'draft'}">
                <td>
                  <!-- Row action menu -->
                          ...
                          <base-button
                            ...
                            @click="() => {
                              receiveInterestModalRef.toggleModal({
                                _id: insurance._id,
                                uuid: insurance.interest_schedule?.uuid,
                                payment_date: insurance.interest_schedule?.payment_date,
                                amount: insurance.interest_schedule?.amount,
                                bank_id: insurance.interest_schedule?.bank?._id,
                                bank_account_uuid: insurance.interest_schedule?.bank?.account?.uuid,
                                received_date: insurance.interest_schedule?.received_date,
                                received_amount: insurance.interest_schedule?.received_amount,
                                additional_bank_id: insurance.interest_schedule?.additional_bank?._id,
                                additional_bank_account_uuid: insurance.interest_schedule?.additional_bank?.account?.uuid,
                                received_additional_payment_date: insurance.interest_schedule?.received_additional_payment_date,
                                received_additional_payment_amount: insurance.interest_schedule?.received_additional_payment_amount,
                                notes: insurance.interest_schedule?.notes,
                                readonly: true
                              })
                ...
                <!-- Insurance fields rendered conditionally based on column visibility -->
                <td v-if="columns['status']?.isVisible">
                  <base-badge v-if="insurance.status === 'draft'" variant="filled" color="danger" class="font-bold w-32 uppercase">
                    <base-icon icon="i-fa7-solid:box-open" /> Draft
                  </base-badge>
                  <base-badge v-else-if="insurance.status === 'active'" variant="filled" color="info" class="font-bold w-32 uppercase">
                    <base-icon icon="i-fa7-solid:box-dollar" /> Active
                  </base-badge>
                  <base-badge v-else-if="insurance.status === 'withdrawn'" variant="filled" color="success" class="font-bold w-32 uppercase">
                    <base-icon icon="i-fa7-solid:box-check" /> Withdrawn
                  </base-badge>
                  <base-badge v-else-if="insurance.status === 'renewed'" variant="filled" color="success" class="font-bold w-32 uppercase">
                ...
                <td v-if="columns['interest_schedule.status']?.isVisible">
                  <base-button
                    @click="receiveInterestModalRef.toggleModal({
                      ...
                      payment_date: insurance.interest_schedule?.payment_date,
                      amount: insurance.interest_schedule?.amount,
                      bank_id: insurance.interest?.bank?._id,
                      bank_account_uuid: insurance.interest?.bank?.account?.uuid,
                    })"
                    v-if="authStore.hasPermission('insurances:receive-interest')
                      && (!insurance.interest_schedule?.received_amount || insurance.interest_schedule?.received_amount === 0)
                      && (insurance.status === 'active' || insurance.status === 'withdrawn')"
                    variant="filled"
                    color="primary"
                  ...
                  <base-badge v-else-if="insurance.interest_schedule?.received_date && (insurance.interest_schedule?.received_amount ?? 0) > 0 && insurance.status !== 'draft'" variant="filled" color="success" class="w-32 font-bold">
                    <base-icon icon="i-fa7-solid:box-check" /> RECEIVED
                  </base-badge>
                </td>
                <td v-if="columns['placement.policy_number']?.isVisible">{{ insurance.placement?.policy_number }}</td>
                ...
                <td class="text-right" v-if="columns['interest_schedule.amount']?.isVisible">{{ formatNumber(insurance.interest_schedule?.amount, 4) }}</td>
                <td class="text-right" v-if="columns['interest_schedule.received_amount']?.isVisible">{{ formatNumber(insurance.interest_schedule?.received_amount, 4) }}</td>
                <td class="text-right" v-if="columns['interest_schedule.received_additional_payment_amount']?.isVisible">{{ formatNumber(insurance.interest_schedule?.received_additional_payment_amount, 4) }}</td>
                <td class="text-right" v-if="columns['interest_schedule.remaining_amount']?.isVisible">{{ formatNumber(roundNumber((insurance.interest_schedule?.amount ?? 0) - (insurance.interest_schedule?.received_amount ?? 0) - (insurance.interest_schedule?.received_additional_payment_amount ?? 0), 4), 4) }}</td>
                ...
                <td class="text-right" v-if="columns['interest_schedule.additional_bank.name']?.isVisible">{{ insurance.interest_schedule?.additional_bank?.name }}</td>
                <td class="text-right" v-if="columns['interest_schedule.additional_bank.account.account_number']?.isVisible">{{ insurance.interest_schedule?.additional_bank?.account?.account_number }}</td>
                <td class="text-right" v-if="columns['interest_schedule.additional_bank.account.account_name']?.isVisible">{{ insurance.interest_schedule?.additional_bank?.account?.account_name }}</td>
                <td class="text-right" v-if="columns['interest_schedule.created_by.username']?.isVisible">{{ insurance.interest_schedule?.created_by?.username }}</td>
                <td v-if="columns['form_number']?.isVisible">
                  <router-link :to="`/insurances/${insurance._id}`" class="text-blue">{{ insurance.form_number }}</router-link>
                </td>
                <td v-if="columns['owner.name']?.isVisible">
                  <router-link :to="`/master/owners/${insurance.owner?._id}`" class="text-blue">{{ insurance.owner?.name }}</router-link>
                </td>
                ...
                <td v-if="columns['placement.term']?.isVisible">{{ formatNumber(insurance.placement?.term, 4) }}</td>
