...
import { restoreDepositApi } from '@/composables/api/deposits/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreDepositApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Deposit "${label.value}" success`, { color: 'success' });
    emit('restored');
