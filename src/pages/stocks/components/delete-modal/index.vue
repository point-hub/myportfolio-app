...
import { deleteStockApi } from '@/composables/api/stocks/delete.api';
...
const onDelete = async (reason: string) => {
  ...
  try {
    await deleteStockApi(_id.value as string, reason);
    toast(`Delete Stock "${label.value}" success`, { color: 'success' });
    emit('deleted');
  } catch (error) {
