...
import { deleteBankApi } from '@/composables/api/master/banks/delete.api';
...
const onDelete = async (reason: string) => {
  ...
  try {
    await deleteBankApi(_id.value as string, reason);
    toast(`Delete Bank "${label.value}" success`, { color: 'success' });
    emit('deleted');
  } catch (error) {
