...
import { updateOwnerApi } from '@/composables/api/master/owners/update.api';
...
const update = async () => {
  try {
    isSaving.value = true;
    const response = await updateOwnerApi(route.params.id as string, form.data);
    if (response?.matched_count) {
      toast('Update success', { color: 'success' });
      await router.push(`/master/owners/${route.params.id}`);
    }
  } catch (error) {
    const errorResponse = handleError(error);
