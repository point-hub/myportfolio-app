...
import { getSavingCashbacksApi, type ISavingData } from '@/composables/api/savings/get-cashbacks.api';
...
const getSavings = async (page = 1) => {
  try {
    ...
    const response = await getSavingCashbacksApi({
      search: filter,
            ...
            <template v-for="(saving, index) in savings" :key="index">
              <tr :class="{'bg-red-50 dark:bg-red-800': saving.status === 'draft'}">
                          ...
                          <base-button
                            ...
                            @click="() => {
                              receiveCashbackModalRef.toggleModal({
                                ...
                                bank_account_uuid: saving.cashback_schedule?.bank?.account?.uuid,
                                received_date: saving.cashback_schedule?.received_date,
                                received_amount: saving.cashback_schedule?.received_amount,
                                additional_bank_id: saving.cashback_schedule?.additional_bank?._id,
                                additional_bank_account_uuid: saving.cashback_schedule?.additional_bank?.account?.uuid,
                                received_additional_payment_date: saving.cashback_schedule?.received_additional_payment_date,
                                received_additional_payment_amount: saving.cashback_schedule?.received_additional_payment_amount,
                                notes: saving.cashback_schedule?.notes,
                                readonly: true
                              })
                ...
                <td v-if="columns['cashback_schedule.status']?.isVisible" class="text-center">
                  ...
                  <base-badge v-else-if="(saving.cashback_schedule?.received_amount ?? 0) > 0 && saving.status !== 'draft'" variant="filled" color="success" class="font-bold w-32">
                ...
                <td class="text-right" v-if="columns['cashback_schedule.amount']?.isVisible">{{ formatNumber(saving.cashback_schedule?.amount, 4) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.received_amount']?.isVisible">{{ formatNumber(saving.cashback_schedule?.received_amount, 4) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.received_additional_payment_amount']?.isVisible">{{ formatNumber(saving.cashback_schedule?.received_additional_payment_amount, 4) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.remaining_amount']?.isVisible">{{ formatNumber(roundNumber((saving.cashback_schedule?.amount ?? 0) - (saving.cashback_schedule?.received_amount ?? 0) - (saving.cashback_schedule?.received_additional_payment_amount ?? 0), 4), 4) }}</td>
                ...
                <td class="text-right" v-if="columns['cashback_schedule.additional_bank.name']?.isVisible">{{ saving.cashback_schedule?.additional_bank?.name }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.additional_bank.account.account_number']?.isVisible">{{ saving.cashback_schedule?.additional_bank?.account?.account_number }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.additional_bank.account.account_name']?.isVisible">{{ saving.cashback_schedule?.additional_bank?.account?.account_name }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.created_by.username']?.isVisible">{{ saving.cashback_schedule?.created_by?.username }}</td>
