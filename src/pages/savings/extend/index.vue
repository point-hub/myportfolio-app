...
import { extendSavingApi } from '@/composables/api/savings/extend.api';
...
const onSave = async () => {
  try {
    isSaving.value = true;
    const response = await extendSavingApi(`${route.params.id}`, form.data);
    if (response?.inserted_id) {
      toast('Extend success', { color: 'success' });
      await router.push(`/savings/${response?.inserted_id}`);
    }
  } catch (error) {
    const errorResponse = handleError(error);
