...
import { archivePaymentStockApi } from '@/composables/api/payment-stocks/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archivePaymentStockApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Payment Stock "${label.value}" success`, { color: 'success' });
    emit('archived');
