...
import { createOwnerApi } from '@/composables/api/master/owners/create.api';
...
const save = async () => {
  try {
    isSaving.value = true;
    const response = await createOwnerApi(form.data);
    if (response?.inserted_id) {
      toast('Create success', { color: 'success' });
      await router.push(`/master/owners/${response.inserted_id}`);
    }
  } catch (error) {
