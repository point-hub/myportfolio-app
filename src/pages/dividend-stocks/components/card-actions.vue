...
const dividendStock = defineModel<IForm>('data');

/**
 * Opens the archive confirmation modal for a specific dividend stock.
 */
const onArchiveModal = () => {
  archiveModalRef.value.toggleModal({
    _id: dividendStock.value?._id,
    label: `${dividendStock.value?.form_number}`,
  });
};
...
/**
 * Opens the restore confirmation modal for a specific dividend stock.
 */
...
const onDeleted = async () => {
  await router.push('/dividend-stocks');
};
</script>
...
<template>
  ...
  <base-card class="py-3! gap-0!">
    <div class="flex gap-2 overflow-auto scrollbar-hidden">
      ...
      <router-link v-if="authStore.hasPermission('dividend-stocks:update') && dividendStock?._id" :to="`/dividend-stocks/${route.params.id}/edit`">
        <base-button variant="filled" color="primary" size="sm" class="font-bold">
          <base-icon icon="i-fa7-solid:file-pen" /> EDIT
        </base-button>
      </router-link>
      <!-- <router-link v-if="authStore.hasPermission('dividend-stocks:read')" :to="`/dividend-stocks/${route.params.id}/histories`">
        <base-button variant="filled" color="primary" size="sm" class="font-bold">
          <base-icon icon="i-fa7-solid:rectangle-history-circle-user" /> HISTORIES
        </base-button>
      </router-link>
      <router-link v-if="authStore.hasPermission('audit-logs:read')" :to="`/dividend-stocks/${route.params.id}/audits`">
        <base-button variant="filled" color="primary" size="sm" class="font-bold">
          <base-icon icon="i-fa7-solid:file-magnifying-glass" /> AUDITS
        </base-button>
      </router-link> -->
      <base-button v-if="authStore.hasPermission('dividend-stocks:delete') && dividendStock?._id" @click="onDeleteModal" variant="filled" color="primary" size="sm" class="font-bold">
        <base-icon icon="i-fa7-solid:trash-xmark" /> DELETE
      </base-button>
      <base-button v-if="authStore.hasPermission('dividend-stocks:update') && dividendStock?._id && !dividendStock.is_archived" @click="onArchiveModal" variant="filled" color="primary" size="sm" class="font-bold">
        <base-icon icon="i-fa7-solid:box-archive" /> ARCHIVE
      </base-button>
      <base-button v-if="authStore.hasPermission('dividend-stocks:update') && dividendStock?._id && dividendStock.is_archived" @click="onRestoreModal" variant="filled" color="primary" size="sm" class="font-bold">
