...
import { archiveDepositApi } from '@/composables/api/deposits/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveDepositApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Deposit "${label.value}" success`, { color: 'success' });
    emit('archived');
