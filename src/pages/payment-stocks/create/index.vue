...
import { createPaymentStockApi } from '@/composables/api/payment-stocks/create.api';
import { draftPaymentStockApi } from '@/composables/api/payment-stocks/draft.api';
...
const onSaveAsDraft = async () => {
  try {
    isSaving.value = true;
    const response = await draftPaymentStockApi(form.data);
    if (response?.inserted_id) {
      toast('Create success', { color: 'success' });
      await router.push(`/payment-stocks/${response.inserted_id}`);
    }
  } catch (error) {
...
const onSave = async () => {
  try {
    isSaving.value = true;
    const response = await createPaymentStockApi(form.data);
    if (response?.inserted_id) {
      toast('Create success', { color: 'success' });
      await router.push(`/payment-stocks/${response.inserted_id}`);
    }
