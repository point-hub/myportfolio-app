          ...
          <template v-if="!isLoading && deposits && deposits.length > 0">
            <template v-for="(deposit, index) in deposits" :key="index">
              <tr :class="{'bg-red-50 dark:bg-red-800': deposit.status === 'draft'}">
                          ...
                          <base-button
                            ...
                            @click="() => {
                              receiveCashbackModalRef.toggleModal({
                                ...
                                received_additional_payment_date: deposit.cashback_schedule?.received_additional_payment_date,
                                received_additional_payment_amount: deposit.cashback_schedule?.received_additional_payment_amount,
                                notes: deposit.cashback_schedule?.notes,
                              })
                              rowMenuRef[index].toggle()
                            }"
                          >
                ...
                <td class="text-right" v-if="columns['cashback_schedule.amount']?.isVisible">{{ formatNumber(deposit.cashback_schedule?.amount, 4) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.received_amount']?.isVisible">{{ formatNumber(deposit.cashback_schedule?.received_amount, 4) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.received_additional_payment_amount']?.isVisible">{{ formatNumber(deposit.cashback_schedule?.received_additional_payment_amount, 4) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.remaining_amount']?.isVisible">{{ formatNumber(roundNumber((deposit.cashback_schedule?.amount ?? 0) - (deposit.cashback_schedule?.received_amount ?? 0) - (deposit.cashback_schedule?.received_additional_payment_amount ?? 0), 4), 4) }}</td>
