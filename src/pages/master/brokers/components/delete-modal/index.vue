...
import { deleteBrokerApi } from '@/composables/api/master/brokers/delete.api';
...
const onDelete = async (reason: string) => {
  ...
  try {
    await deleteBrokerApi(_id.value as string, reason);
    toast(`Delete Broker "${label.value}" success`, { color: 'success' });
    emit('deleted');
  } catch (error) {
