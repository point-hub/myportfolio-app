...
import { createSellBondApi } from '@/composables/api/bonds/create-sell.api';
...
import CardSell from './card-sell.vue';
import CardCoupon from './card-sell.vue';
  ...
  <div class="content-container">
    ...
    <base-card class="py-3! gap-0!">
      <div class="flex gap-2 overflow-auto scrollbar-hidden">
        <base-button @click="isTabSell = false" :class="{
          'bg-primary text-white': !isTabSell,
          'bg-info text-white': isTabSell
        }" size="sm" class="font-bold"
        >
          <base-icon icon="i-fa7-solid:file" /> INFORMATION
        </base-button>
        <base-button @click="isTabSell = true" :class="{
          'bg-primary text-white': isTabSell,
          'bg-info text-white': !isTabSell
        }
        " size="sm" class="font-bold"
        >
          <base-icon icon="i-fa7-solid:file" /> SELL BOND
        </base-button>
      </div>
    </base-card>
    <template v-if="!isTabSell">
    ...
    <template v-else>
      <card-sell v-model:data="form.data" v-model:errors="form.errors" v-model:is-saving="isSaving" />
