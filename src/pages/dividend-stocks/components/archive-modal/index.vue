...
import { archiveDividendStockApi } from '@/composables/api/dividend-stocks/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveDividendStockApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Dividend Stock "${label.value}" success`, { color: 'success' });
    emit('archived');
