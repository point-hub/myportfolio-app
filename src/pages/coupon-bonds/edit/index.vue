...
import { updateBondApi } from '@/composables/api/bonds/update.api';
import { updateDraftBondApi } from '@/composables/api/bonds/update-draft.api';
...
const onUpdate = async () => {
  try {
    isSaving.value = true;
    const response = await updateBondApi(route.params.id as string, form.data);
    if (response?.matched_count) {
      toast('Update success', { color: 'success' });
      await router.push(`/bonds/${route.params.id}`);
    }
  } catch (error) {
    const errorResponse = handleError(error);
...
const onUpdateDraft = async () => {
  try {
    isSaving.value = true;
    const response = await updateDraftBondApi(route.params.id as string, form.data);
    if (response?.matched_count) {
      toast('Update success', { color: 'success' });
      await router.push(`/bonds/${route.params.id}`);
    }
