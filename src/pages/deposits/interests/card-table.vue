...
import { getDepositInterestsApi, type IDepositData } from '@/composables/api/deposits/get-interests.api';
...
const getDeposits = async (page = 1) => {
  try {
    ...
    const response = await getDepositInterestsApi({
      search: filter,
          ...
          <template v-if="!isLoading && deposits && deposits.length > 0">
            <template v-for="(deposit, index) in deposits" :key="index">
              <tr :class="{'bg-red-50 dark:bg-red-800': deposit.status === 'draft'}">
                          ...
                          <base-button
                            ...
                            @click="() => {
                              receiveInterestModalRef.toggleModal({
                                ...
                                bank_account_uuid: deposit.interest_schedule?.bank?.account?.uuid,
                                received_date: deposit.interest_schedule?.received_date,
                                received_amount: deposit.interest_schedule?.received_amount,
                                additional_bank_id: deposit.interest_schedule?.additional_bank?._id,
                ...
                <td class="text-right" v-if="columns['interest_schedule.amount']?.isVisible">{{ formatNumber(deposit.interest_schedule?.amount, 2) }}</td>
                <td class="text-right" v-if="columns['interest_schedule.received_amount']?.isVisible">{{ formatNumber(deposit.interest_schedule?.received_amount, 2) }}</td>
                <td class="text-right" v-if="columns['interest_schedule.received_additional_payment_amount']?.isVisible">{{ formatNumber(deposit.interest_schedule?.received_additional_payment_amount, 2) }}</td>
                <td class="text-right" v-if="columns['interest_schedule.remaining_amount']?.isVisible">{{ formatNumber(roundNumber((deposit.interest_schedule?.amount ?? 0) - (deposit.interest_schedule?.received_amount ?? 0) - (deposit.interest_schedule?.received_additional_payment_amount ?? 0), 2), 2) }}</td>
                ...
                <td v-if="columns['placement.term']?.isVisible">{{ formatNumber(deposit.placement?.term, 2) }}</td>
