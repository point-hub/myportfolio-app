      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && auditLogs && auditLogs.length > 0">
            <tr v-for="(owner, index) in auditLogs" :key="index">
              <!-- Fields rendered conditionally based on column visibility -->
              <td v-if="columns['created_at']?.isVisible" class="w-40">{{ formatDate(owner.created_at) }}</td>
              <td v-if="columns['operation_id']?.isVisible">
                <router-link :to="`/administrator/audit-logs/${owner.operation_id}`" class="text-blue">{{ owner.operation_id?.substring(0, 13) }}...</router-link>
              </td>
              <td v-if="columns['actor']?.isVisible">
                <router-link :to="`/master/users/${owner.actor_id}`" class="text-blue">{{ owner.actor_name }}</router-link>
              </td>
              <td v-if="columns['before']?.isVisible">{{ owner.before }}</td>
              <td v-if="columns['after']?.isVisible">{{ owner.after }}</td>
            </tr>
          </template>
        </tbody>
      </base-table>
