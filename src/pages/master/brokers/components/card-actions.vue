...
const broker = defineModel<IForm>('data');

/**
 * Opens the archive confirmation modal for a specific broker.
 */
const onArchiveModal = () => {
  archiveModalRef.value.toggleModal({
    _id: broker.value?._id,
    label: `[${broker.value?.code}] ${broker.value?.name}`,
  });
};
...
/**
 * Opens the restore confirmation modal for a specific broker.
 */
...
const onDeleted = async () => {
  await router.push('/master/brokers');
};
</script>
...
<template>
  ...
  <base-card class="py-3! gap-0!">
    <div class="flex gap-2 overflow-auto scrollbar-hidden">
      ...
      <router-link v-if="authStore.hasPermission('brokers:update') && broker?._id" :to="`/master/brokers/${route.params.id}/edit`">
        <base-button variant="filled" color="primary" size="sm" class="font-bold">
          <base-icon icon="i-fa7-solid:file-pen" /> EDIT
        </base-button>
      </router-link>
      <router-link v-if="authStore.hasPermission('brokers:read')" :to="`/master/brokers/${route.params.id}/histories`">
        <base-button variant="filled" color="primary" size="sm" class="font-bold">
          <base-icon icon="i-fa7-solid:rectangle-history-circle-user" /> HISTORIES
        </base-button>
      </router-link>
      <router-link v-if="authStore.hasPermission('audit-logs:read')" :to="`/master/brokers/${route.params.id}/audits`">
        <base-button variant="filled" color="primary" size="sm" class="font-bold">
          <base-icon icon="i-fa7-solid:file-magnifying-glass" /> AUDITS
        </base-button>
      </router-link>
      <base-button v-if="authStore.hasPermission('brokers:delete') && broker?._id" @click="onDeleteModal" variant="filled" color="primary" size="sm" class="font-bold">
        <base-icon icon="i-fa7-solid:trash-xmark" /> DELETE
      </base-button>
      <base-button v-if="authStore.hasPermission('brokers:update') && broker?._id && !broker.is_archived" @click="onArchiveModal" variant="filled" color="primary" size="sm" class="font-bold">
        <base-icon icon="i-fa7-solid:box-archive" /> ARCHIVE
      </base-button>
      <base-button v-if="authStore.hasPermission('brokers:update') && broker?._id && broker.is_archived" @click="onRestoreModal" variant="filled" color="primary" size="sm" class="font-bold">
