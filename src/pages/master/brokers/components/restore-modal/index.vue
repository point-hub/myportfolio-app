...
import { restoreBrokerApi } from '@/composables/api/master/brokers/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreBrokerApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Broker "${label.value}" success`, { color: 'success' });
    emit('restored');
