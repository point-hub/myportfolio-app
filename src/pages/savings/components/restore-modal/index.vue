...
import { restoreSavingApi } from '@/composables/api/savings/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreSavingApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Saving "${label.value}" success`, { color: 'success' });
    emit('restored');
