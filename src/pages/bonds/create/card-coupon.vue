...
watchEffect(() => {
  data.value.coupon_gross_amount = roundNumber((data.value.principal_amount ?? 0) * ((data.value.coupon_rate ?? 0) / 100) / (data.value.base_date ?? 0) * (data.value.coupon_tenor ?? 0), 2);
...
<template>
  <base-card title="Coupon Information">
    <div class="flex flex-col gap-4">
      <base-input layout="horizontal" label="Coupon Tenor" :model-value="formatNumber(data.coupon_tenor, 2)" disabled />
      ...
      <base-input layout="horizontal" label="Next Coupon Date" align="left" :model-value="nextCouponDate" disabled decimal-length="2" />
