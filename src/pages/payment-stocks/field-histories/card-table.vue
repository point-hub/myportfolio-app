      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && auditLogs && auditLogs.length > 0">
            <tr v-for="(paymentStock, index) in auditLogs" :key="index">
              <!-- Fields rendered conditionally based on column visibility -->
              <td v-if="columns['created_at']?.isVisible" class="w-40">{{ formatDate(paymentStock.created_at) }}</td>
              <td v-if="columns['operation_id']?.isVisible">
                <router-link :to="`/administrator/audit-logs/${paymentStock.operation_id}`" class="text-blue">{{ paymentStock.operation_id?.substring(0, 13) }}...</router-link>
              </td>
              <td v-if="columns['actor']?.isVisible">
                <router-link :to="`/master/users/${paymentStock.actor_id}`" class="text-blue">{{ paymentStock.actor_name }}</router-link>
              </td>
              <td v-if="columns['before']?.isVisible">{{ paymentStock.before }}</td>
              <td v-if="columns['after']?.isVisible">{{ paymentStock.after }}</td>
            </tr>
          </template>
        </tbody>
      </base-table>
