...
import { archiveBondApi } from '@/composables/api/bonds/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveBondApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Bond "${label.value}" success`, { color: 'success' });
    emit('archived');
