...
import { getDepositsApi, type IDepositData } from '@/composables/api/deposits/get.api';
 ...
 * @param deposit - The data row to delete
 * @param index - Index of the row for UI references
 */
const onDeleteModal = (deposit: IDepositData, index: number) => {
  rowMenuRef.value[index].toggle(false);
  deleteModalRef.value.toggleModal({
    _id: deposit._id,
    label: `${deposit.form_number}`,
  });
};
          ...
          <template v-if="!isLoading && deposits && deposits.length > 0">
            <tr v-for="(deposit, index) in deposits" :key="index" :class="{'bg-red-50 dark:bg-red-800': deposit.status === 'draft'}">
                ...
                <base-popover placement="bottom" ref="rowMenuRef">
                  ...
                  <template #content>
                    <base-card class="p-0! gap-0! -mt-2" shadow>
                      <div class="flex flex-col">
                        <router-link :to="`/deposits/${deposit._id}`">
                        ...
                        <router-link v-if="authStore.hasPermission('deposits:update')" :to="`/deposits/${deposit._id}/edit`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
              ...
              <td v-if="columns['withdrawal.status']?.isVisible">
                ...
                <base-badge v-else-if="deposit.withdrawal?.received_date && deposit.withdrawal?.remaining_amount && deposit.withdrawal?.remaining_amount <= 0" variant="filled" color="success" class="font-bold w-32 uppercase">
              ...
              <td v-if="columns['group.name']?.isVisible">
                <router-link :to="`/master/owners/${deposit.group?._id}`" class="text-blue">{{ deposit.group?.name }}</router-link>
              </td>
              ...
              <td v-if="columns['withdrawal.received_amount']?.isVisible">{{ formatNumber(deposit.withdrawal?.received_amount, 4) }}</td>
              <td v-if="columns['withdrawal.remaining_amount']?.isVisible">{{ formatNumber(deposit.withdrawal?.remaining_amount, 4) }}</td>
