          ...
          <template v-if="!isLoading && savings && savings.length > 0">
            <template v-for="(saving, index) in savings" :key="index">
              <tr :class="{'bg-red-50 dark:bg-red-800': saving.status === 'draft'}">
                ...
                <td class="text-right" v-if="columns['interest_schedule.amount']?.isVisible">{{ formatNumber(saving.interest_schedule?.amount, 3) }}</td>
                <td class="text-right" v-if="columns['interest_schedule.received_amount']?.isVisible">{{ formatNumber(saving.interest_schedule?.received_amount, 3) }}</td>
                <td class="text-right" v-if="columns['interest_schedule.received_additional_payment_amount']?.isVisible">{{ formatNumber(saving.interest_schedule?.received_additional_payment_amount, 3) }}</td>
                <td class="text-right" v-if="columns['interest_schedule.remaining_amount']?.isVisible">{{ formatNumber(roundNumber((saving.interest_schedule?.amount ?? 0) - (saving.interest_schedule?.received_amount ?? 0) - (saving.interest_schedule?.received_additional_payment_amount ?? 0), 3), 3) }}</td>
                ...
                <td v-if="columns['placement.term']?.isVisible">{{ formatNumber(saving.placement?.term, 3) }}</td>
