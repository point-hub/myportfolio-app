...
import { getIssuersApi, type IIssuerData } from '@/composables/api/master/issuers/get.api';
 ...
 * - issuers data retrieved from API
 * - loading state
...
const onPageUpdate = async () => {
  if (!isInitialSetup.value) {
    await getIssuers(pagination.page);
 ...
 * @param issuer - The data row to delete
 * @param index - Index of the row for UI references
 */
const onDeleteModal = (issuer: IIssuerData, index: number) => {
    ...
    <div class="flex flex-col gap-4">
      <base-table>
        ...
        <tbody>
          ...
          <!-- Show no data found message if no issuers and query params exist -->
          <tr v-if="!isLoading && issuers?.length === 0 && route.query">
            <td :colspan="countVisibleColumns + 1">
          ...
          <!-- Render rows of issuer data when available -->
          <template v-if="!isLoading && issuers && issuers.length > 0">
            <tr v-for="(issuer, index) in issuers" :key="index">
              <td>
                <!-- Row action menu -->
                  ...
                  <template #content>
                    <base-card class="p-0! gap-0! -mt-2" shadow>
                      <div class="flex flex-col">
                        <router-link :to="`/master/issuers/${issuer._id}`">
                        ...
                        <router-link v-if="authStore.hasPermission('issuers:update')" :to="`/master/issuers/${issuer._id}/edit`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
                        ...
                        <base-divider orientation="vertical" class="my-0!" />
                        <base-button v-if="authStore.hasPermission('issuers:delete')" @click="onDeleteModal(issuer, index)" variant="text" color="danger" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
              ...
              <!-- Issuer fields rendered conditionally based on column visibility -->
              <td v-if="columns['code']?.isVisible">
                <router-link :to="`/master/issuers/${issuer._id}`" class="text-blue">{{ issuer.code }}</router-link>
              </td>
              <td v-if="columns['name']?.isVisible">
                <router-link :to="`/master/issuers/${issuer._id}`" class="text-blue">{{ issuer.name }}</router-link>
              </td>
              <td v-if="columns['notes']?.isVisible">{{ issuer.notes }}</td>
              <td v-if="columns['is_archived']?.isVisible">
                <base-badge v-if="issuer.is_archived" variant="filled" color="danger" class="font-bold">
