      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && auditLogs && auditLogs.length > 0">
            <tr v-for="(broker, index) in auditLogs" :key="index">
              <!-- Fields rendered conditionally based on column visibility -->
              <td v-if="columns['created_at']?.isVisible" class="w-40">{{ formatDate(broker.created_at) }}</td>
              <td v-if="columns['operation_id']?.isVisible">
                <router-link :to="`/administrator/audit-logs/${broker.operation_id}`" class="text-blue">{{ broker.operation_id?.substring(0, 13) }}...</router-link>
              </td>
              <td v-if="columns['actor']?.isVisible">
                <router-link :to="`/master/users/${broker.actor_id}`" class="text-blue">{{ broker.actor_name }}</router-link>
              </td>
              <td v-if="columns['before']?.isVisible">{{ broker.before }}</td>
              <td v-if="columns['after']?.isVisible">{{ broker.after }}</td>
            </tr>
          </template>
        </tbody>
      </base-table>
