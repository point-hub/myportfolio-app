      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && auditLogs && auditLogs.length > 0">
            <tr v-for="(deposit, index) in auditLogs" :key="index">
              <!-- Fields rendered conditionally based on column visibility -->
              <td v-if="columns['created_at']?.isVisible" class="w-40">{{ formatDate(deposit.created_at) }}</td>
              <td v-if="columns['operation_id']?.isVisible">
                <router-link :to="`/administrator/audit-logs/${deposit.operation_id}`" class="text-blue">{{ deposit.operation_id?.substring(0, 13) }}...</router-link>
              </td>
              <td v-if="columns['actor']?.isVisible">
                <router-link :to="`/master/users/${deposit.actor_id}`" class="text-blue">{{ deposit.actor_name }}</router-link>
              </td>
              <td v-if="columns['before']?.isVisible">{{ deposit.before }}</td>
              <td v-if="columns['after']?.isVisible">{{ deposit.after }}</td>
            </tr>
          </template>
        </tbody>
      </base-table>
