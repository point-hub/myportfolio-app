        ...
        <tbody>
          ...
          <template v-if="!isLoading && bonds && bonds.length > 0">
            <tr v-for="(bond, index) in bonds" :key="index" :class="{'bg-red-50 dark:bg-red-800': bond.status === 'draft'}">
              ...
              <td v-if="columns['principal_amount']?.isVisible">{{ formatNumber(bond.principal_amount, 4) }}</td>
              <td v-if="columns['remaining_amount']?.isVisible">{{ formatNumber(bond.remaining_amount, 4) }}</td>
              <td v-if="columns['proceed_amount']?.isVisible">{{ formatNumber(bond.proceed_amount, 4) }}</td>
              <td v-if="columns['accrued_interest']?.isVisible">{{ formatNumber(bond.accrued_interest, 4) }}</td>
              <td v-if="columns['total_proceed']?.isVisible">{{ formatNumber(bond.total_proceed, 4) }}</td>
              <td v-if="columns['selling_price']?.isVisible">{{ formatNumber(bond.selling_price, 4) }}</td>
              ...
              <td v-if="columns['disbursement_amount']?.isVisible">{{ formatNumber(bond.disbursement_amount, 4) }}</td>
              <td v-if="columns['disbursement_amount_received']?.isVisible">{{ formatNumber(bond.disbursement_amount_received, 4) }}</td>
              <td v-if="columns['disbursement_amount_remaining']?.isVisible">{{ formatNumber((bond.disbursement_amount ?? 0) - (bond.disbursement_amount_received ?? 0), 4) }}</td>
