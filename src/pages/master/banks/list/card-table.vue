...
import { getBanksApi, type IBankData } from '@/composables/api/master/banks/get.api';
...
const onPageUpdate = async () => {
  if (!isInitialSetup.value) {
    await getBanks(pagination.page);
    ...
    <div class="flex flex-col gap-4">
      <base-table>
        ...
        <tbody>
          ...
          <!-- Show no data found message if no banks and query params exist -->
          <tr v-if="!isLoading && banks?.length === 0 && route.query">
            <td :colspan="countVisibleColumns + 1">
          ...
          <!-- Render rows of bank data when available -->
          <template v-if="!isLoading && banks && banks.length > 0">
            <tr v-for="(bank, index) in banks" :key="index">
                  ...
                  <template #content>
                    <base-card class="p-0! gap-0! -mt-2" shadow>
                      <div class="flex flex-col">
                        <router-link :to="`/master/banks/${bank._id}`">
                        ...
                        <router-link v-if="authStore.hasPermission('banks:update')" :to="`/master/banks/${bank._id}/edit`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
              ...
              <!-- Bank fields rendered conditionally based on column visibility -->
              <td v-if="columns['code']?.isVisible">
                <router-link :to="`/master/banks/${bank._id}`" class="text-blue">{{ bank.code }}</router-link>
              </td>
              <td v-if="columns['name']?.isVisible">
                <router-link :to="`/master/banks/${bank._id}`" class="text-blue">{{ bank.name }}</router-link>
              </td>
