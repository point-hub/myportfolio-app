...
import { getOwnersApi, type IOwnerData } from '@/composables/api/master/owners/get.api';
 ...
 * - owners data retrieved from API
 * - loading state
...
const onPageUpdate = async () => {
  if (!isInitialSetup.value) {
    await getOwners(pagination.page);
...
/**
 * Opens the delete confirmation modal for a specific owner.
 * Also closes the row menu popover.
 * @param owner - The data row to delete
 * @param index - Index of the row for UI references
 */
const onDeleteModal = (owner: IOwnerData, index: number) => {
  ...
  <base-card title="Owners">
    <div class="flex flex-col lg:flex-row gap-2 items-center justify-between mb-8">
      ...
      <div class="flex gap-1">
        <router-link v-if="authStore.hasPermission('owners:create')" to="/master/owners/create">
    ...
    <div class="flex flex-col gap-4">
      <base-table>
        ...
        <tbody>
          ...
          <!-- Show no data found message if no owners and query params exist -->
          <tr v-if="!isLoading && owners?.length === 0 && route.query">
            <td :colspan="countVisibleColumns + 1">
          ...
          <!-- Render rows of owner data when available -->
          <template v-if="!isLoading && owners && owners.length > 0">
            <tr v-for="(owner, index) in owners" :key="index">
                  ...
                  <template #content>
                    <base-card class="p-0! gap-0! -mt-2" shadow>
                      <div class="flex flex-col">
                        <router-link :to="`/master/owners/${owner._id}`">
                        ...
                        <router-link v-if="authStore.hasPermission('owners:update')" :to="`/master/owners/${owner._id}/edit`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
                        ...
                        <base-divider orientation="vertical" class="my-0!" />
                        <base-button v-if="authStore.hasPermission('owners:delete')" @click="onDeleteModal(owner, index)" variant="text" color="danger" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
              ...
              <!-- Owner fields rendered conditionally based on column visibility -->
              <td v-if="columns['code']?.isVisible">
                <router-link :to="`/master/owners/${owner._id}`" class="text-blue">{{ owner.code }}</router-link>
              </td>
              <td v-if="columns['name']?.isVisible">
                <router-link :to="`/master/owners/${owner._id}`" class="text-blue">{{ owner.name }}</router-link>
              </td>
              <td v-if="columns['notes']?.isVisible">{{ owner.notes }}</td>
              <td v-if="columns['is_archived']?.isVisible">
                <base-badge v-if="owner.is_archived" variant="filled" color="danger" class="font-bold">
