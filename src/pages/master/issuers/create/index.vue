...
import { createIssuerApi } from '@/composables/api/master/issuers/create.api';
...
const save = async () => {
  try {
    isSaving.value = true;
    const response = await createIssuerApi(form.data);
    if (response?.inserted_id) {
      toast('Create success', { color: 'success' });
      await router.push(`/master/issuers/${response.inserted_id}`);
    }
  } catch (error) {
