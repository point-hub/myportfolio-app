      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && auditLogs && auditLogs.length > 0">
            <tr v-for="(insurance, index) in auditLogs" :key="index">
              <!-- Fields rendered conditionally based on column visibility -->
              <td v-if="columns['created_at']?.isVisible" class="w-40">{{ formatDate(insurance.created_at) }}</td>
              <td v-if="columns['operation_id']?.isVisible">
                <router-link :to="`/administrator/audit-logs/${insurance.operation_id}`" class="text-blue">{{ insurance.operation_id?.substring(0, 13) }}...</router-link>
              </td>
              <td v-if="columns['actor']?.isVisible">
                <router-link :to="`/master/users/${insurance.actor_id}`" class="text-blue">{{ insurance.actor_name }}</router-link>
              </td>
              <td v-if="columns['before']?.isVisible">{{ insurance.before }}</td>
              <td v-if="columns['after']?.isVisible">{{ insurance.after }}</td>
            </tr>
          </template>
        </tbody>
      </base-table>
