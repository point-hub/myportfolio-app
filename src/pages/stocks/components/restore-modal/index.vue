...
import { restoreStockApi } from '@/composables/api/stocks/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreStockApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Stock "${label.value}" success`, { color: 'success' });
    emit('restored');
