...
import { getInsurancesApi, type IInsuranceData } from '@/composables/api/insurances/get.api';
...
const getInsurances = async (page = 1) => {
  try {
    ...
    const response = await getInsurancesApi({
      search: filter,
 ...
 * @param insurance - The data row to delete
 * @param index - Index of the row for UI references
 */
const onDeleteModal = (insurance: IInsuranceData, index: number) => {
  rowMenuRef.value[index].toggle(false);
  deleteModalRef.value.toggleModal({
    _id: insurance._id,
    label: `${insurance.form_number}`,
  });
};
...
const getWithdrawalAmount = (insurance: IInsuranceData) => {
      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && insurances && insurances.length > 0">
            <tr v-for="(insurance, index) in insurances" :key="index" :class="{'bg-red-50 dark:bg-red-800': insurance.status === 'draft'}">
              <td>
                <!-- Row action menu -->
                  ...
                  <template #content>
                    <base-card class="p-0! gap-0! -mt-2" shadow>
                      <div class="flex flex-col">
                        <router-link :to="`/insurances/${insurance._id}`">
                        ...
                        <router-link v-if="authStore.hasPermission('insurances:update')" :to="`/insurances/${insurance._id}/edit`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
                            <base-icon icon="i-fa7-light-file-pen" />
                            <p class="flex-1">Edit</p>
                          </base-button>
                        </router-link>
                        <router-link v-if="authStore.hasPermission('insurances:renew') && insurance.status === 'active'" :to="`/insurances/${insurance._id}/extend`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
                            <base-icon icon="i-fa7-light-file-pen" />
                        ...
                        <base-button v-if="authStore.hasPermission('insurances:delete')" @click="onDeleteModal(insurance, index)" variant="text" color="danger" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
              ...
              <td v-if="columns['withdrawal.status']?.isVisible">
                <base-badge v-if="insurance.withdrawal?.received_date && insurance.withdrawal?.remaining_amount && insurance.withdrawal?.remaining_amount > 0" variant="filled" color="danger" class="font-bold w-32 uppercase">
                  <base-icon icon="i-fa7-solid:box-open" /> Outstanding
                </base-badge>
                <base-badge v-else-if="insurance.withdrawal?.received_date && insurance.withdrawal?.remaining_amount && insurance.withdrawal?.remaining_amount <= 0" variant="filled" color="success" class="font-bold w-32 uppercase">
              ...
              <td v-if="columns['group.name']?.isVisible">
                <router-link :to="`/master/owners/${insurance.group?._id}`" class="text-blue">{{ insurance.group?.name }}</router-link>
              </td>
              ...
              <td v-if="columns['withdrawal.received_amount']?.isVisible">{{ formatNumber(insurance.withdrawal?.received_amount, 2) }}</td>
              <td v-if="columns['withdrawal.remaining_amount']?.isVisible">{{ formatNumber(insurance.withdrawal?.remaining_amount, 2) }}</td>
