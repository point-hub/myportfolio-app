...
import { archiveBankApi } from '@/composables/api/master/banks/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveBankApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Bank "${label.value}" success`, { color: 'success' });
    emit('archived');
