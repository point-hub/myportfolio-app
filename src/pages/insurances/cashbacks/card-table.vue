...
import { getInsuranceCashbacksApi, type IInsuranceData } from '@/composables/api/insurances/get-cashbacks.api';
...
const getInsurances = async (page = 1) => {
  try {
    ...
    const response = await getInsuranceCashbacksApi({
      search: filter,
          ...
          <template v-if="!isLoading && insurances && insurances.length > 0">
            <template v-for="(insurance, index) in insurances" :key="index">
              <tr :class="{'bg-red-50 dark:bg-red-800': insurance.status === 'draft'}">
                    ...
                    <template #content>
                      <base-card class="p-0! gap-0! -mt-2" shadow>
                        <div class="flex flex-col">
                          <base-button
                            ...
                            @click="() => {
                              receiveCashbackModalRef.toggleModal({
                                _id: insurance._id,
                                uuid: insurance.cashback_schedule?.uuid,
                                payment_date: insurance.cashback_schedule?.payment_date,
                                amount: insurance.cashback_schedule?.amount,
                                bank_id: insurance.cashback_schedule?.bank?._id,
                                bank_account_uuid: insurance.cashback_schedule?.bank?.account?.uuid,
                                received_date: insurance.cashback_schedule?.received_date,
                                received_amount: insurance.cashback_schedule?.received_amount,
                                additional_bank_id: insurance.cashback_schedule?.additional_bank?._id,
                                additional_bank_account_uuid: insurance.cashback_schedule?.additional_bank?.account?.uuid,
                                received_additional_payment_date: insurance.cashback_schedule?.received_additional_payment_date,
                                received_additional_payment_amount: insurance.cashback_schedule?.received_additional_payment_amount,
                                notes: insurance.cashback_schedule?.notes,
                                readonly: true
                              })
                          ...
                          <base-button
                            v-if="authStore.hasPermission('insurances:receive-cashback')"
                            variant="text"
                            color="info"
                            class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!"
                            @click="() => {
                ...
                <td v-if="columns['cashback_schedule.status']?.isVisible" class="text-center">
                  <base-button
                    @click="receiveCashbackModalRef.toggleModal({
                      ...
                      payment_date: insurance.cashback_schedule?.payment_date,
                      amount: insurance.cashback_schedule?.amount,
                      bank_id: insurance.cashback?.bank?._id,
                      bank_account_uuid: insurance.cashback?.bank?.account?.uuid,
                    })"
                    v-if="authStore.hasPermission('insurances:receive-cashback')
                      && (!insurance.cashback_schedule?.received_amount || insurance.cashback_schedule?.received_amount === 0)
                  ...
                  <base-badge v-else-if="(insurance.cashback_schedule?.received_amount ?? 0) > 0 && insurance.status !== 'draft'" variant="filled" color="success" class="font-bold w-32">
                ...
                <td class="text-left" v-if="columns['cashback_schedule.payment_date']?.isVisible">{{ insurance.cashback_schedule?.payment_date }}</td>
                <td class="text-left" v-if="columns['cashback_schedule.received_date']?.isVisible">{{ insurance.cashback_schedule?.received_date }}</td>
                <td class="text-left" v-if="columns['cashback_schedule.received_additional_payment_date']?.isVisible">{{ insurance.cashback_schedule?.received_additional_payment_date }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.amount']?.isVisible">{{ formatNumber(insurance.cashback_schedule?.amount, 2) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.received_amount']?.isVisible">{{ formatNumber(insurance.cashback_schedule?.received_amount, 2) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.received_additional_payment_amount']?.isVisible">{{ formatNumber(insurance.cashback_schedule?.received_additional_payment_amount, 2) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.remaining_amount']?.isVisible">{{ formatNumber(roundNumber((insurance.cashback_schedule?.amount ?? 0) - (insurance.cashback_schedule?.received_amount ?? 0) - (insurance.cashback_schedule?.received_additional_payment_amount ?? 0), 2), 2) }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.bank.name']?.isVisible">{{ insurance.cashback_schedule?.bank?.name }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.bank.account.account_number']?.isVisible">{{ insurance.cashback_schedule?.bank?.account?.account_number }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.bank.account.account_name']?.isVisible">{{ insurance.cashback_schedule?.bank?.account?.account_name }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.additional_bank.name']?.isVisible">{{ insurance.cashback_schedule?.additional_bank?.name }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.additional_bank.account.account_number']?.isVisible">{{ insurance.cashback_schedule?.additional_bank?.account?.account_number }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.additional_bank.account.account_name']?.isVisible">{{ insurance.cashback_schedule?.additional_bank?.account?.account_name }}</td>
                <td class="text-right" v-if="columns['cashback_schedule.created_by.username']?.isVisible">{{ insurance.cashback_schedule?.created_by?.username }}</td>
