...
import { deleteInsuranceApi } from '@/composables/api/insurances/delete.api';
...
const onDelete = async (reason: string) => {
  ...
  try {
    await deleteInsuranceApi(_id.value as string, reason);
    toast(`Delete Insurance "${label.value}" success`, { color: 'success' });
    emit('deleted');
  } catch (error) {
