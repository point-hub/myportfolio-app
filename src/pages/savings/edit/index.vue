...
import { findSavingApi } from '@/composables/api/savings/find-by-id.api';
import { updateSavingApi } from '@/composables/api/savings/update.api';
import { updateDraftSavingApi } from '@/composables/api/savings/update-draft.api';
...
onMounted(async () => {
  try {
    ...
    const response = await findSavingApi(route.params.id as string);
    if (response) {
...
const onUpdate = async () => {
  try {
    isSaving.value = true;
    const response = await updateSavingApi(route.params.id as string, form.data);
    if (response?.matched_count) {
      toast('Update success', { color: 'success' });
      await router.push(`/savings/${route.params.id}`);
    }
  } catch (error) {
    const errorResponse = handleError(error);
...
const onUpdateDraft = async () => {
  try {
    isSaving.value = true;
    const response = await updateDraftSavingApi(route.params.id as string, form.data);
    if (response?.matched_count) {
      toast('Update success', { color: 'success' });
      await router.push(`/savings/${route.params.id}`);
    }
