...
import { archiveOwnerApi } from '@/composables/api/master/owners/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveOwnerApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Owner "${label.value}" success`, { color: 'success' });
    emit('archived');
  } catch (error) {
