...
import { deleteBondApi } from '@/composables/api/bonds/delete.api';
...
const onDelete = async (reason: string) => {
  ...
  try {
    await deleteBondApi(_id.value as string, reason);
    toast(`Delete Bond "${label.value}" success`, { color: 'success' });
    emit('deleted');
  } catch (error) {
