...
watchEffect(() => {
  data.value.remaining_amount = roundNumber((data.value.principal_amount ?? 0) - (data.value.disbursement_amount ?? 0), 4);
  data.value.disbursement_amount_remaining = roundNumber((data.value.disbursement_amount ?? 0) - (data.value.disbursement_amount_received ?? 0), 4);
});
