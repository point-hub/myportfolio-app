      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && auditLogs && auditLogs.length > 0">
            <tr v-for="(stock, index) in auditLogs" :key="index">
              <!-- Fields rendered conditionally based on column visibility -->
              <td v-if="columns['created_at']?.isVisible" class="w-40">{{ formatDate(stock.created_at) }}</td>
              <td v-if="columns['operation_id']?.isVisible">
                <router-link :to="`/administrator/audit-logs/${stock.operation_id}`" class="text-blue">{{ stock.operation_id?.substring(0, 13) }}...</router-link>
              </td>
              <td v-if="columns['actor']?.isVisible">
                <router-link :to="`/master/users/${stock.actor_id}`" class="text-blue">{{ stock.actor_name }}</router-link>
              </td>
              <td v-if="columns['before']?.isVisible">{{ stock.before }}</td>
              <td v-if="columns['after']?.isVisible">{{ stock.after }}</td>
            </tr>
          </template>
        </tbody>
      </base-table>
