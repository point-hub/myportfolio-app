...
import { restoreInsuranceApi } from '@/composables/api/insurances/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreInsuranceApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Insurance "${label.value}" success`, { color: 'success' });
    emit('restored');
