...
import { restoreBondApi } from '@/composables/api/bonds/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreBondApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Bond "${label.value}" success`, { color: 'success' });
    emit('restored');
